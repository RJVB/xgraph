...
LocalWin *ActiveWin= NULL, HO_PreviousWin, *HO_PreviousWin_ptr= NULL;
...
#if (defined(__MACH__) || defined(__APPLE_CC__)) && FFTW_CYCLES_PER_SEC == 1
...
/* #if defined(__GNUC__) && (defined(__MACH__) || defined(__APPLE_CC__)) && FFTW_CYCLES_PER_SEC == 1	*/
	...
	{ struct mach_timebase_info timebase;
		...
		if( debugFlag && Mach_Absolute_Time_Factor!= MACH_ABSOLUTE_TIME_FACTOR ){
			fprintf( StdErr, "Warning: precompiled time calibration %g different from runtime value %g (diff=%g)!\n",
