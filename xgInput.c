...
{  char *PAGER= getenv("PAGER");
   ...
   extern char *fodstrs[];
	...
	if( !ArgsParsed ){
		...
		fputs( "         [-python[Version] [<python file>]] [-pyversion[Version]] [-preserve_ftime] [-IO_Import <module> <file>] [-registry]\n", fp);
		...
		if( Check_Option( strncasecmp, argv[idx], "-help", 5)== 0 ){
		  ...
		  char c;
			...
			/* The next string is a set name */
				...
				if( Check_Option( strcmp, argv[idx], "-PSm", -1)==0 ){
				...
				else if( Check_Option( strncasecmp, argv[idx], "-pyversion", 10)==0 ){
				  int pv = -1;
				  extern DM_Python_Interface *dm_python;
				  extern int UsePythonVersion;
				  	if( argv[idx][10] != '\0' ){
						if( !dm_python && sscanf( &argv[idx][10], "%d", &pv ) == 1 && pv > 0 ){
							UsePythonVersion = pv;
						}
					}
					idx += 1;
				  ...
				  extern int Init_Python(), UsePythonVersion;
				  	if( argv[idx][7] != '\0' ){
						if( !dm_python && sscanf( &argv[idx][7], "%d", &pv ) == 1 && pv > 0 ){
							UsePythonVersion = pv;
						}
					}
					if( idx+1 >= argc ){
