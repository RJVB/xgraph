...
int idrawInit(strm, width, height, title_family, title_size,
            legend_family, legend_size, label_family, label_size, axis_family, axis_size, flags, win_info, errmsg, initFile )
	...
	char *legend_family;		/* Name of label font family   */
	double legend_size;		/* label font height (points)  */
	char *label_family;		/* Name of label font family   */
	double label_size;		/* label font height (points)  */
 ...
 */
{
	char	**l;
	double	scx, scy;
	xgOut *out_info= &(win_info->dev_info);		/* Device info (RETURN)       */
	Info	*idraw_info= (Info*) out_info->user_state;

	if( !idraw_info ){
		idraw_info = (Info *) calloc( 1, sizeof(*idraw_info));
	}
	...
	out_info -> errortick_len = axis_size/2;
	out_info -> axis_height = axis_size;
	out_info -> legend_height = legend_size;
	out_info -> label_height = label_size;
	out_info -> title_height = title_size;
	out_info -> axis_width = (axis_size*5.0) / 12.0;
	out_info -> legend_width = (legend_size*5.0) / 12.0;
	out_info -> label_width = (label_size*5.0) / 12.0;
 ...
 */
{
	...
	FIX(y);
	font = (style == T_AXIS || style== T_LABEL || style== T_LEGEND )? idraw -> axis_font:
		idraw -> title_font;
	size = (style == T_AXIS || style== T_LABEL || style== T_LEGEND ) ? idraw -> axis_size:
		idraw -> title_size;
 ...
 */
{
	Info	*idraw = (Info *) info->dev_info.user_state;
