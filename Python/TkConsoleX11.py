...
class Console(Frame):
	...
	def destroy(self):
		self.install_idle_callback(None)
		self.update()
		if( self.idle_cancel ):
			print >>sys.stderr, 'Cancelled after_idle callback'
		Frame.destroy(self)
	...
	def install_idle_callback(self, callback = None):
		if callback == None:
			if self.idle_cancel:
				self.idle_callback = None
				#self.after_cancel(self.idle_cancel)
				self.idle_cancel = None
			else:
				self.idle_callback = None
		else:
			self.idle_callback = callback
			#self.idle_cancel = self.after_idle(callback,self)
			self.idle_cancel = self.after(self.idle_interval,callback,self)
...
# 		if not self.idle_callback == None:
# 			self.install_idle_callback(self.idle_callback)
...
def TkConsole(D={},idle_callback=None,idle_interval=100):
	c = Console(dict=D)
	c.dict["console"] = c
	c.pack(fill=BOTH, expand=1)
	c.master.title("Python Console v%s" % VERSION)
	c.idle_callback = idle_callback
	c.idle_interval = idle_interval
	if not idle_callback == None:
		c.update()
		c.install_idle_callback(idle_callback)
	mainloop()
